...
let LANGUAGECODE = "";
let TIMEZONE = "";
const tokenExpiresAfter = 240;	//	The access token may expire after about 300 seconds, sometimes earlier.
			...
			if (error && error.response && error.response.error)  {
				...
				if (error.response.status && typeof(error.response.status) == "number") {
					...
					switch (error.response.status) {
						case 502:
							//	Issue #7: Don't let adapter terminate itself for 50x API-errors
							errMsg = "(502) Bad gateway";
							break;

						default:
							// @ts-ignore
							errMsg = `(${error.response.status}) ${error.response.error}`;
							break;
					}
					parentAdapter.log.error(`Failed to login to Xplora Cloud service: ${errMsg}`);
					parentAdapter.log.debug(`(DETAILS) Failed to login to Xplora Cloud service: ${error}`);
				}
				else {
					//	Error but no status code
					// @ts-ignore
					parentAdapter.log.error(`Failed to login to Xplora Cloud service: ${error.response.error}`);
					parentAdapter.log.debug(`(DETAILS) Failed to login to Xplora Cloud service: ${error}`);
				}
			}
			else {
